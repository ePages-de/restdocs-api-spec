...
class OpenApi3GeneratorTest {
    ...
    fun `should aggregate responses with different content type and different response schema`() {
        ...
        then(
            openApiJsonPathContext.read<Any>("$productPatchByIdPath.requestBody.content.application/json-patch+json.schema.\$ref"),
        ).isNotNull()
        then(
            openApiJsonPathContext.read<Any>("$productPatchByIdPath.requestBody.content.application/json-patch+json.examples.test-1"),
        ).isNotNull()
        ...
        then(
            openApiJsonPathContext.read<Any>("$productPatchByIdPath.responses.200.content.application/hal+json.examples.test-1"),
        ).isNotNull()
    ...
    fun `should aggregate equal schemas across operations`() {
        ...
        val patchResponseSchemaRef =
            openApiJsonPathContext.read<String>(
                "paths./products/{id}.patch.responses.200.content.application/json.schema.\$ref",
            )
        val getResponseSchemaRef =
            openApiJsonPathContext.read<String>(
                "paths./products/{id}.get.responses.200.content.application/json.schema.\$ref",
            )
    ...
    fun thenResourceHasValidSchemaGeneratedFromRequestParameters(method: String) {
        ...
        val getResponseSchemaRef =
            openApiJsonPathContext.read<String>(
                "$productGetByIdPath.requestBody.content.application/x-www-form-urlencoded.schema.\$ref",
            )
        val schemaId = getResponseSchemaRef.removePrefix("#/components/schemas/")
        then(openApiJsonPathContext.read<String>("components.schemas.$schemaId.properties.locale.type")).isEqualTo("string")
        then(
            openApiJsonPathContext.read<String>("components.schemas.$schemaId.properties.locale.description"),
    ...
    fun thenResourceHasFormDataInRequestBodyAndNotAsQueryParameters(method: String) {
        ...
        then(
            openApiJsonPathContext.read<List<String>>(
    ...
    fun thenGetProductByIdOperationIsValid() {
        ...
        then(
            openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'id')].schema.type"),
        ...
        then(
            openApiJsonPathContext.read<List<Boolean>>("$productGetByIdPath.parameters[?(@.name == 'locale')].required"),
        ).containsOnly(false)
        then(
            openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'locale')].schema.type"),
        ).containsOnly("string")
        then(openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'locale')].schema.default")).isEmpty()
        then(
            openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'Authorization')].in"),
        ).containsOnly("header")
        then(
            openApiJsonPathContext.read<List<Boolean>>("$productGetByIdPath.parameters[?(@.name == 'Authorization')].required"),
        ).containsOnly(true)
        then(
            openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'Authorization')].example"),
        ).containsOnly("some example")
        then(
            openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'Authorization')].schema.type"),
        ).containsOnly("string")
        then(
            openApiJsonPathContext.read<List<String>>("$productGetByIdPath.parameters[?(@.name == 'Authorization')].schema.default"),
        ).isEmpty()
        ...
        then(
            openApiJsonPathContext.read<List<List<String>>>("$productGetByIdPath.security[*].oauth2").flatMap { it },
        ).containsOnly("prod:r")
    }
    ...
    private fun getProductPatchRequest(
        schema: Schema? = null,
    ...
    private fun getOAuth2SecurityRequirement() =
        SecurityRequirements(
            type = SecurityType.OAUTH2,
            requiredScopes = listOf("prod:r"),
        )

    private fun getJWTSecurityRequirement() =
        SecurityRequirements(
