...
class OpenApi20GeneratorTest {
        ...
        with(openapi.securityDefinitions) {
            ...
            then(this["oauth2"])
                .isEqualToComparingFieldByField(
                    OAuth2Definition()
                        .accessCode("http://example.com/authorize", "http://example.com/token")
    ...
    private fun whenExtractOrFindSchema(
        schemaNameAndSchemaMap: MutableMap<Model, String>,
        ordersSchema: Model,
        shopsSchema: Model,
    ) {
    ...
    private fun thenOptionsRequestExist(
        openapi: Swagger,
    ...
    private fun thenHeadRequestExist(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
    ...
    private fun thenPathParametersExist(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
    ...
    private fun thenGetProductWith200ResponseIsGenerated(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        ...
        then(
            successfulGetResponse
                .examples
                .get(successfulGetProductModel.response.contentType),
    ...
    private fun thenGetProductWith200ResponseIsGeneratedWithDefaultValue(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        val successfulGetProductModel = api[0]
        val productPath = openapi.paths.getValue(successfulGetProductModel.request.path)
        @Suppress("UNCHECKED_CAST")
        thenParametersForGetMatchWithDefaultValue(
            productPath.get.parameters as List<AbstractSerializableParameter<*>>,
            successfulGetProductModel.request,
        )
    }

    private fun thenGetProductWith200ResponseIsGeneratedWithEnumValues(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        val successfulGetProductModel = api[0]
        val productPath = openapi.paths.getValue(successfulGetProductModel.request.path)
        @Suppress("UNCHECKED_CAST")
        thenParametersForGetMatchWithEnumValues(
            productPath.get.parameters as List<AbstractSerializableParameter<*>>,
            successfulGetProductModel.request,
        )
    }

    private fun thenPostProductWith200ResponseIsGenerated(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        ...
        then(
            successfulPostResponse!!
                .examples
                .get(successfulPostProductModel.response.contentType),
        ).isEqualTo(successfulPostProductModel.response.example)
        @Suppress("UNCHECKED_CAST")
        thenParametersForPostMatch(
            productPath.post.parameters as List<AbstractSerializableParameter<*>>,
            successfulPostProductModel.request,
        )
    ...
    private fun thenRequestAndResponseSchemataAreReferenced(
        ...
        val requestBody =
            productPath.post.parameters
                .filter { it.`in` == "body" }
                .first() as BodyParameter
    ...
    private fun thenPostRequestShouldHaveFormDataParameters(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        ...
        @Suppress("UNCHECKED_CAST")
        thenParameterMatches(
            productPath.post.parameters as List<AbstractSerializableParameter<*>>,
            "formData",
            productResourceModel.request.formParameters[0],
        )
    }

    private fun thenPutRequestShouldHaveFormDataParameters(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        val productResourceModel = api[0]
        ...
        @Suppress("UNCHECKED_CAST")
        thenParameterMatches(
            productPath.put.parameters as List<AbstractSerializableParameter<*>>,
            "formData",
            productResourceModel.request.formParameters[0],
        )
    }

    private fun thenGetProductWith400ResponseIsGenerated(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
        ...
        then(
            productPath.get.responses
                .get(badGetProductModel.response.status.toString())!!
                .examples
                .get(badGetProductModel.response.contentType),
    ...
    private fun thenParametersForGetMatch(
    ...
    private fun thenParametersForGetMatchWithEnumValues(
        parameters: List<AbstractSerializableParameter<*>>,
        request: RequestModel,
    ) {
        thenParameterMatches(parameters, "path", request.pathParameters[0])
    ...
    private fun thenParametersForPostMatch(
        parameters: List<AbstractSerializableParameter<*>>,
        request: RequestModel,
    ) {
        thenParameterMatches(parameters, "header", request.headers[0])
    ...
    private fun findParameterByTypeAndName(
        parameters: List<AbstractSerializableParameter<*>>,
        type: String,
        name: String,
    ): AbstractSerializableParameter<*>? =
        parameters.firstOrNull {
            it.`in` == type && it.name == name
        }

    private fun thenDeleteProductIsGenerated(
        openapi: Swagger,
        api: List<ResourceModel>,
    ) {
    ...
    private fun thenEnumValuesAreSetInRequestAndResponse(openapi: Swagger) {
        ...
        then(
            (openapi.definitions["ProductRequest"]!!.properties["someEnum"] as StringProperty).enum,
        ).containsExactly("FIRST_VALUE", "SECOND_VALUE", "THIRD_VALUE")
        then(openapi.definitions["ProductResponse"]?.properties?.keys ?: emptyList()).contains("someEnum")
        then(
            (openapi.definitions["ProductResponse"]!!.properties["someEnum"] as StringProperty).enum,
        ).containsExactly("FIRST_VALUE", "SECOND_VALUE", "THIRD_VALUE")
    }
    ...
    private fun postProduct200Response(
        example: String,
        schema: Schema? = null,
    ): ResponseModel =
        ResponseModel(
    ...
    private fun postProductRequest(
        schema: Schema? = null,
        path: String = "/products",
    ): RequestModel =
        RequestModel(
