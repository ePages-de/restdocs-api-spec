...
internal object ConstraintResolver {
    ...
    private val REQUIRED_CONSTRAINTS =
        setOf("javax.validation.constraints.NotNull")
    ...
    internal fun maybeMinSizeArray(fieldDescriptor: FieldDescriptor?) =
        fieldDescriptor?.maybeSizeConstraint()?.let {
            it.configuration["min"] as? Int
        }

    internal fun maybeMaxSizeArray(fieldDescriptor: FieldDescriptor?) =
        fieldDescriptor?.maybeSizeConstraint()?.let {
            it.configuration["max"] as? Int
        }
    ...
    internal fun maybePattern(fieldDescriptor: FieldDescriptor?) =
        fieldDescriptor?.maybePatternConstraint()?.let {
            it.configuration["regexp"] as? String
        }
    ...
    private fun findConstraints(fieldDescriptor: FieldDescriptor): List<Constraint> = fieldDescriptor.attributes.validationConstraints
}
