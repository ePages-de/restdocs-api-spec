...
class JsonSchemaFromFieldDescriptorsGenerator {
    fun generateSchema(
        ...
        val jsonFieldPaths =
            reduceFieldDescriptors(fieldDescriptors)
        ...
        if (schema is ObjectSchema) {
            ...
            if (groups.keys.size == 1 && groups.keys.contains("[]")) {
                ...
                return takeIf {
                    ...
                    ?: ArraySchema
                        .builder()
                        .allItemSchema(schema.propertySchemas["[]"])
    ...
    private fun isDirectMatch(traversedSegments: List<String>): Predicate<JsonFieldPath> {
        ...
        return Predicate { jsonFieldPath ->
            ...
            remainingSegments.isEmpty() || remainingSegments.size == 1 &&
                JsonFieldPath.isArraySegment(
                    remainingSegments[0],
                )
        }
    }
    ...
    private fun groupFieldsByFirstRemainingPathSegment(
        traversedSegments: List<String>,
        jsonFieldPaths: List<JsonFieldPath>,
    ): Map<String, List<JsonFieldPath>> = jsonFieldPaths.groupBy { it.remainingSegments(traversedSegments)[0] }
    ...
    internal class FieldDescriptorWithSchemaType(
        ...
        private val jsonSchemaPrimitiveTypes: Set<String> =
            setOf(
        ...
        fun jsonSchemaType(): Schema {
            ...
            return if (schemaBuilders.size == 1) {
                ...
                oneOf(schemaBuilders.map { it.build() }).description(description).checkNullable().build()
            }
        }
        ...
        companion object {
            ...
            private fun jsonSchemaPrimitiveTypeFromDescriptorType(fieldDescriptorType: String) =
                fieldDescriptorType
                    .lowercase()
                    .let {
                        if (it == "varies") "empty" else it
                    } // varies is used by spring rest docs if the type is ambiguous - in json schema we want to represent as empty
        }
    }
}

private fun StringSchema.Builder.applyConstraints(fieldDescriptor: FieldDescriptor) =
    apply {
...
private fun ArraySchema.Builder.applyConstraints(fieldDescriptor: FieldDescriptor?) =
    apply {
        minItems(maybeMinSizeArray(fieldDescriptor))
        maxItems(maybeMaxSizeArray(fieldDescriptor))
    }

private fun NumberSchema.Builder.applyConstraints(fieldDescriptor: FieldDescriptor) =
    apply {
